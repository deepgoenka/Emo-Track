% Features extracted based on RAVDESS Dataset

% Specify the directory containing your WAV files
dataset_directory = "C:\Users\User\Downloads\All Datasets\RAVDESS_Dataset";

% Get a list of all WAV files in the directory
wav_files = dir(fullfile(dataset_directory, '*.wav'));

label_mapping = containers.Map([1, 2, 3, 4, 5, 6, 7, 8], ...
...
for i = 1:length(wav_files)
    ...
    current_file_path = fullfile(dataset_directory, wav_files(i).name);

    % Extract the label from the file name
    emotion_label = label_mapping(str2double(wav_files(i).name(7:8))); % Assumes the label is at positions 7-8
    disp(emotion_label)
...
csvwrite('RAVDESS_Features.csv', all_statistics);